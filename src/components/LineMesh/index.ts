...
const vs: string = require('./vertex.glsl');
const fs: string = require('./fragment.glsl');
...
export default class LineMesh extends Drawable {
    ...
    constructor(private points: Vector2[], private _width: number) {
        super(vs, fs, gl.TRIANGLES);
        const length = this.points.length;
        this.a_vertex = new Attribute(gl.ARRAY_BUFFER, new Float32Array(length * 2 * 2), gl.FLOAT, 2);
        ...
        this._drawType = gl.TRIANGLE_STRIP;
    }
